#!/usr/bin/env python3
"""
anchor_sheet_v0_3.py

Constellation Track B, step B4 support. Successor to v0.2.

New in v0.3
    --tax-basis {stated,implied}   strike historical NOPLAT at the stated statutory rate
                                   or at each year's implied effective rate (open item T1)
    --non-scaling-stock S          subtract S from otherOperatingAssets before ratioing,
                                   so the scaling remainder can be inspected for stability
                                   before the split is committed (open item O1)
...
Usage
    python3 scenario/anchor_sheet_v0_3.py --model evals/case-01/expected.json
    python3 scenario/anchor_sheet_v0_3.py --model evals/case-01/expected.json \
...
def main():
    ...
    ap.add_argument("--tax-basis", choices=["stated", "implied"], default="stated",
                    help="basis for historical NOPLAT. Default stated (assumptions.taxRate)")
    ap.add_argument("--non-scaling-stock", type=float, default=0.0,
                    help="portion of otherOperatingAssets held flat, in model units")
    ...
    for t in range(n):
        ...
        eff = i["incomeTaxes"][t] / pretax if pretax else None
        rate = tax_stated if args.tax_basis == "stated" or eff is None else eff
        noplat = ebit * (1.0 - rate)
        ...
        rows.append({
            ...
            "effective_tax_rate_implied": eff, "noplatTaxRateUsed": rate,
            "other_op_assets_scaling_pct_revenue": ((other - args.non_scaling_stock) / rev
                                                    if rev else None),
    ...
    print("NOPLAT tax basis: %s   non-scaling stock subtracted from otherOA: %.1f"
          % (args.tax_basis, args.non_scaling_stock))
    ...
    if args.non_scaling_stock:
        print("\n  scaling remainder of otherOA, as a share of revenue "
              "(stable is the test that the split is right)")
        for r in rows:
            print("  %-6s %s" % (r["year"], pct(r["other_op_assets_scaling_pct_revenue"])))
